"""Windows hotkey backend adapter with pynput integration."""
...
class WindowsHotkeyBackend(InMemoryHotkeyBackend):
    """Windows hotkey adapter with OS-level registration via pynput.

    Falls back to in-memory behavior when pynput is unavailable or
    registration fails (e.g., missing permissions).
    """
    ...
    def is_available(cls) -> bool:
        ...
        if not _pynput_available:
            return False
        # On Windows, pynput should generally work
        return True
